...
describe('Logger', () => {
  ...
  it('should log messages correctly - pino', () => {
    ...
    const dl = l.defaultLogger;

    dl.debug('Debug message', { extra: 'debug' });
    dl.info('Info message', { extra: 'info' });
    dl.warn('Warn message', { extra: 'warn' });
    dl.error('Error message', { extra: 'error' });
